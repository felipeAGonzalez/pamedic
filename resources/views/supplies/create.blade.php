    ...
    <form method="POST" action="{{ route('supplies.store') }}">
        ...
        <div class="mb-3">
            ...
            <select name="for_vascular_access" class="form-control @error('for_vascular_access') is-invalid @enderror" required>
                <option value="">-- Seleccionar --</option>
                <option value="catheter"  {{ old('for_vascular_access') == 'catheter'  ? 'selected' : '' }}>Catéter</option>
                <option value="fistula"   {{ old('for_vascular_access') == 'fistula'   ? 'selected' : '' }}>Fístula</option>
                <option value="both"      {{ old('for_vascular_access') == 'both'      ? 'selected' : '' }}>Ambos</option>
                <option value="no_apply"  {{ old('for_vascular_access') == 'no_apply'  ? 'selected' : '' }}>No Aplica</option>
            </select>
